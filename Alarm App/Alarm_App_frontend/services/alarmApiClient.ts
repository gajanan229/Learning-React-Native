...
const EXPO_PUBLIC_ALARM_BACKEND_URL = process.env.EXPO_PUBLIC_ALARM_BACKEND_URL;

if (!EXPO_PUBLIC_ALARM_BACKEND_URL) {
    console.error("CRITICAL: EXPO_PUBLIC_ALARM_BACKEND_URL is not defined. Please check your .env file and expo-env.d.ts");
}

const alarmApiClient = axios.create({
    baseURL: EXPO_PUBLIC_ALARM_BACKEND_URL,
    headers: {
...
alarmApiClient.interceptors.request.use(
...
alarmApiClient.interceptors.response.use(
    ...
    (error: AxiosError) => {
        ...
        if (response) {
            ...
            if (response.status === 401) {
                ...
                console.warn('Received 401 from Alarm API. Attempting to log out user.');
                // Ensure logout also clears SecureStore token
                useAuthStore.getState().logout(); 
            } else if (response.status === 403) {
                // Forbidden: User is authenticated but doesn't have permission
                console.warn('Received 403 Forbidden from Alarm API.');
